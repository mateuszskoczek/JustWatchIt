...
public abstract class QueryParameters<T> : QueryParameters
{
    #region CONSTANTS

    public abstract IDictionary<string, Func<T, IComparable?>>? OrderKeys { get; }
    ...
    public IEnumerable<T> PrepareData(IEnumerable<T> data)
    {
        ...
        {
            if (OrderKeys is not null && OrderKeys.TryGetValue(OrderBy, out Func<T, IComparable?>? orderFunc))
            {
                data = Order == "asc" ? data.OrderBy(_ => orderFunc) : data.OrderByDescending(_ => orderFunc);
            }
        }
